...
use crate::controller::{command_handler::{Command, CommandOutput, CommandSender}, game_object::{self, GameObject}, sound, sprite, undo::UndoAction};
#[cfg(not(feature = "verify"))]
use crate::controller::loc::data::*;
#[cfg(not(feature = "verify"))]
use crate::controller::menu::editor::context_menu::ContextMenuItem;
use super::{simple_object::{Holdable, ObjectType}, Level, LevelObject, LevelTheme, SubObjectDeleteUndoAction, AABB};

pub const LAYER: u32 = super::L_PHYSICS_OBJECT;
...
impl LevelObject for Teleporter {
	...
	fn create_editor_view(&self, command_sender: &mut dyn CommandOutput, _level: &Level) -> Vec<GameObject> {
		let mut a = GameObject::new(game_object::OBJ_NO_ANIM, 5);
		a.x = (self.x1 * 32) as f32;
		a.y = (self.y1 * 32) as f32;
		a.create(command_sender);
		a.set_sprite(command_sender, sprite::SUKIMA + 1);

		let mut b = GameObject::new(game_object::OBJ_NO_ANIM, 5);
		b.x = (self.x2 * 32) as f32;
		b.y = (self.y2 * 32) as f32;
		b.create(command_sender);
		b.set_sprite(command_sender, sprite::SUKIMA);
	...
	fn ghost_sub_object(&self, command_sender: &mut CommandSender, sub_object: usize, objects: &mut Vec<GameObject>) -> ((i32, i32), i32) {
		...
		(
			...
			sprite::SUKIMA + 1 - sub_object as i32
		)
	}
	...
	fn simple_object_type(&self) -> Option<ObjectType> {
		Some(ObjectType::Holdable(Holdable::SukimaBall))
	}
}
